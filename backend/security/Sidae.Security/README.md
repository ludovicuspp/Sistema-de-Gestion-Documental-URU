...
git clone <url-del-repositorio> Sidae.Security
cd Sidae.Security
...
- **Debug:** `bin/Debug/net10.0/Sidae.Security.dll`
- **Release:** `bin/Release/net10.0/Sidae.Security.dll`
...
Desde `Sidae.Security/Sidae.Security.csproj`, la ruta relativa al proyecto compartido es:
...
Estructura esperada en disco: `backend/Sidae.Commons/` y `backend/security/Sidae.Security/` (hermanos bajo `backend/`).
...
3. **Referenciar** en `Sidae.Security.csproj`:
